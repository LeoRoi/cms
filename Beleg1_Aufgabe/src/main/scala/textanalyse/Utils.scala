...
object Utils extends Serializable {
  ...
  def getData(fileName: String, sc: SparkContext): RDD[(String, String)] = {
    ...
    val out = rawdata.filter(_._2 == 1).map(_._1.asInstanceOf[(String, String)]).cache

    println(">> getData output:")
    out.take(3).foreach(println)
    println(">> getData output _1:")
    out.map(_._1).take(3).foreach(println)
    println(">> getData output _2:")
    out.map(_._2).take(3).foreach(println)
  ...
  def getGoldStandard(fileName: String, sc: SparkContext): RDD[(String, String)] = {
    ...
    val failed = rawdata.filter(_._2 == -1).cache
    if (failed.count > 0) {
      println("Failed GoldStandard to parse lines:"); failed.take(10).foreach(println)
    }
